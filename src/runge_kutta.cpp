...
Geometry::CloudOfPoints Numeric::solve_RK4_movable_vortices(double dt, CartesianGridOfSpeed& t_velocity, Simulation::Vortices& t_vortices, Geometry::CloudOfPoints const& t_points)
{
    constexpr double onesixth = 1./6.;
    using vector = Simulation::Vortices::vector;
    using point  = Simulation::Vortices::point;
    ...
    for (std::size_t iVortex=0; iVortex<t_vortices.numberOfVortices(); ++iVortex)
    {
        ...
        newVortexCenter[iVortex] = t_velocity.updatePosition(p + onesixth*dt*(v1+2.*v2+2.*v3+v4));
